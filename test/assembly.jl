...
@testset "assembly linearized system matrix and rhs" begin
    ...
    A = assembleLinearSystemMatrix(node_vector=node_vector, num_ele=num_ele, ndofs=ndofs, costFunc_constant=costFunc_constant, cross_section_area=bar_area);

    rhs_lin = assembleRhsLinearBar(data_star=data_star, node_vector=node_vector, num_ele=num_ele, ndofs=ndofs, costFunc_constant=costFunc_constant, bar_distF=bar_distF, cross_section_area=bar_area);

    # initial guess
    x = zeros(ndof_tot);

    # assembly linearized system and rhs
    rhs = assembleBalanceResidual(x=x, data_star=data_star, node_vector=node_vector, num_ele=num_ele, ndofs=ndofs, costFunc_constant=costFunc_constant, bar_distF=bar_distF, cross_section_area=bar_area);

    J = assembleLinearizedSystemMatrix(x=x, node_vector=node_vector, num_ele=num_ele, ndofs=ndofs, costFunc_constant=costFunc_constant, cross_section_area=bar_area);
